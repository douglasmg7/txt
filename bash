...
exec 3<> testfile		# fd3 in and out to testfile
exec 3>&-			# close fd3
tee filename 	# send to STDOUT and filename
...
# list open files descriptions
# -p process id
# -d file description
# -a and for to options
lsof
lsof -a -p $$ -d 0,1,2
...
# bash ignores any SIGQUIT (3) and SIGTERM (15)
# bash not ignores SIGHUP (1) and SIGINT (2)

# shell refers to each process running in the shell as a job and assigns each job a unique job number within the current shell
...
nohup ./test1.sh &
	# to accommodate any output generated by the command, the nohup command automatically redirects STDOUT and STDERR messages to a file, called nohup.out.

# job control - function of starting, stopping, killing, and resuming jobs

# start a stopped job in background
bg
	# start default job in background [3]+ (with sign +)
bg 2
